...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupItemsRecyclerView() {
        val spanCount = 3
        itemsRecyclerView.layoutManager = StaggeredGridLayoutManager(spanCount, StaggeredGridLayoutManager.VERTICAL)
        val adapter = ItemsAdapter(categories)
        itemsRecyclerView.adapter = adapter
    }

    private fun scrollToCategory(position: Int) {
        val layoutManager = itemsRecyclerView.layoutManager as StaggeredGridLayoutManager
        var itemPosition = 0
